...
def inference_result():
    ...
    model_path='/home/arsc/tmp/pycharm_project_698/DA_Road/run_model_experiments/our_model/DeepGlobe/experiment_20240408_044748/149_checkpoint.pth'#修改(2)
    ...
    out_path = '../test_output/DeepGlobe/OurModel/'  #+ out_id #修改(3)
    if not os.path.exists(out_path):
        try:
            os.mkdir(out_path)
        except FileExistsError:
            pass
    base_dir='/home/arsc/tmp/pycharm_project_639/CSA/data/deepglobe/train'
    with open(base_dir + '/val_crops.txt') as fv:
        test_imgnames = [f.strip() for f in fv.readlines()]
    #加载测试集并测试
    count=0
    for i, imgname in enumerate(test_imgnames):
        count+=1
        imgname=imgname.strip()
        img_path=os.path.join(base_dir+'/crops/images',imgname)
        label_path=os.path.join(base_dir+'/crops/gt',imgname)
        skeleton_path = os.path.join(base_dir + '/crops/skeleton', imgname)
        ...
        imgs, labels, skeletons = imgs.cuda(non_blocking=True), labels.cuda(non_blocking=True), skeletons.cuda(
            non_blocking=True)
        with torch.no_grad():
            loss_total, labeled_seg_pred, labeled_skeleton_pred = model(labeled_img=imgs, labeled_seg_mask=labels, labeled_ske_mask=skeletons,
                                                                        unlabeled_img=None, mode='sup_only')
        # 输出通道是2时
        ...
        print(imgname, val_IoU)
        with open('/home/arsc/tmp/pycharm_project_698/DA_Road/test_output/DeepGlobe/img_record.txt', 'a') as f:
            f.write(imgname + ' ' + str(val_IoU) + '\n')
