...
class ObjDataset(Dataset):
    ...
    def __getitem__(self, index):
        image_path= self.train_dir_root+'/train_crops/'+ self.train_images[index]
        gt_path = self.train_dir_root + '/train_labels_crops/' + self.train_images[index]
        skeleton_path = self.train_dir_root + '/train_skeletons_crops/' + self.train_images[index]
...
class ValObjDataset(Dataset):
    ...
    def __getitem__(self, index):
        val_image_path=self.val_dir_root+'/val_crops/'+self.val_images[index]
        val_gt_path=self.val_dir_root+'/val_labels_crops/'+self.val_images[index]
        val_skeleton_path = self.val_dir_root + '/val_skeletons_crops/' + self.val_images[index]
...
def massachusetts_dataset(dir_root, image_size=512, labeled_ratio=0.2):
    train_images = [f for f in os.listdir(dir_root + '/train_crops/') if f.endswith('.jpg') or f.endswith('.png')]
    val_images = [f for f in os.listdir(dir_root + '/val_crops/') if f.endswith('.jpg') or f.endswith('.png')]
...
if __name__ =='__main__':
    massachusetts_dataset(dir_root='/home/arsc/tmp/pycharm_project_639/CSA/data/Massachusetts/crops/',image_size=512, labeled_ratio=0.2)
