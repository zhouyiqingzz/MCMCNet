...
class ObjDataset(Dataset):
    ...
    def __getitem__(self, index):
        image_path= self.train_dir_root+'/train/images/'+ self.train_images[index]
        gt_path = self.train_dir_root + '/train/gt/' + self.train_images[index].split('_')[0] + '_mask.png'
        skeleton_path = self.train_dir_root + '/train/skeleton/' + self.train_images[index].split('_')[0] + '_mask.png'
...
class ValObjDataset(Dataset):
    ...
    def __getitem__(self, index):
        val_image_path=self.val_dir_root+'/val/images/'+self.val_images[index]
        val_gt_path=self.val_dir_root+'/val/gt/'+self.val_images[index].split('_')[0] + '_mask.png'
        val_skeleton_path = self.val_dir_root + '/val/skeleton/' + self.val_images[index].split('_')[0] + '_mask.png'
...
def chn6_dataset(dir_root, image_size=512, labeled_ratio=0.2):
    train_images = [f for f in os.listdir(dir_root + '/train/images') if f.endswith('.jpg') or f.endswith('.png')]
    val_images = [f for f in os.listdir(dir_root + '/val/images') if f.endswith('.jpg') or f.endswith('.png')]
...
if __name__ =='__main__':
    chn6_dataset(dir_root='/home/arsc/tmp/pycharm_project_639/CSA/data/CHN6/',image_size=512, labeled_ratio=0.2)#image为.jpg格式,gt为.png格式
