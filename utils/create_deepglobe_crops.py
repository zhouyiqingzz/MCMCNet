...
def CreatCrops(base_dir, dataset, crop_type, size, stride, image_suffix, gt_suffix):#base_dir=data/deepglobe/train

    crops = os.path.join(base_dir, 'crops')#'data/deepglobe/train/crops'
    if not os.path.exists(crops):
        os.mkdir(crops)
        os.mkdir(crops+'/images')#'data/deepglobe/train/crops/images'
        os.mkdir(crops+'/gt')#'data/deepglobe/train/crops/gt'
    crops_file = open(os.path.join(base_dir,'{}_crops.txt'.format(crop_type)),'w')#'data/deepglobe/train/train_crops.txt'

    full_file_path = os.path.join(base_dir,'{}.txt'.format(crop_type))#'data/deepglobe/train/train.txt'
    full_file = open(full_file_path,'r')

    def get_num_lines(file_path):
        fp = open(file_path, "r+")
        buf = mmap.mmap(fp.fileno(), 0)#mmap实现两个不同进程共享内存,fileio:文件描述符,length=0:要映射文件部分的大小，值为0则映射整个文件
        lines = 0
        while buf.readline():
            lines += 1
        return lines
    ...
    for name in tqdm(full_file, ncols=100, desc="{}_crops".format(crop_type),
        ...
        if dataset == 'SpaceNet':
            ...
            image_file = os.path.join(base_dir, 'images', name + '_sat.jpg')#'data/deepglobe/train/images/某某数字_sat.jpg'
            gt_file = os.path.join(base_dir, 'gt', name + '_mask.png')      #'data/deepglobe/train/gt/某某数字_mask.png'
        ...
        for x in range(maxx+1):
            for y in range(maxy+1):
                ...
                cv2.imwrite(crops+'/images/{}_{}_{}.png'.format(name,x,y),  im_)#'data/deepglobe/train/crops/images/某某数字_0_1.png'
...
def main():
    ...
    parser.add_argument('-d', '--base_dir', type=str, default='../../data/deepglobe/train',
